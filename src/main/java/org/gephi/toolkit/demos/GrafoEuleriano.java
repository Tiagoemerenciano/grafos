...
public class GrafoEuleriano {
    ...
    public static boolean verificaGraus(DirectedGraph dg, Node v){
        return dg.getOutDegree(v) == dg.getInDegree(v);
    }
    ...
    public static void main (String[] args){
        ...
        String retorno;
        if (!verificaGraus(directedGraph, v0)) {
            retorno = "O grafo não é euleriano.";
        } else if (!verificaGraus(directedGraph, v1)) {
            retorno = "O grafo não é euleriano.";
        } else if (!verificaGraus(directedGraph, v2)) {
            retorno = "O grafo não é euleriano.";
        } else if (!verificaGraus(directedGraph, v3)) {
            retorno = "O grafo não é euleriano.";
        } else if (!verificaGraus(directedGraph, v4)) {
            retorno = "O grafo não é euleriano.";
        } else {
            Object vertAnterior = v0;
            boolean primeira = false;
            for(Edge e : directedGraph.getEdges()) {
                if (primeira && vertAnterior != e.getSource().getId()) {
                    retorno = "O grafo não é euleriano.";
                    break;
                }
                primeira = true;
                vertAnterior = e.getTarget().getId();
            }
            retorno = "\nO grafo é euleriano.\n";
        }
